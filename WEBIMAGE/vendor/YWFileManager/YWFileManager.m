...
#import "YWFileManager.h"

@implementation YWFileManager
...
+(void)attributeOfItemAtPath:(NSString *)path forKey:(NSString *)key success:(void(^)(id result))success faild:(void(^)(NSError *error))faild
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        id successResult = [[self attributesOfItemAtPath:path error:&resultError] objectForKey:key];
        if (resultError) {
            if (faild) {
...
+(void)attributesOfItemAtPath:(NSString *)path success:(void(^)(NSDictionary *result))success faild:(void(^)(NSError *error))faild
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError *resultError = nil;
        NSDictionary *resultDic = [[NSFileManager defaultManager] attributesOfItemAtPath:[self absolutePath:path] error:&resultError];
        if (resultError) {
            if (faild) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    faild(resultError);
...
+(void)copyItemAtPath:(NSString *)path toPath:(NSString *)toPath success:(void(^)())success faild:(void(^)(NSError *error))faild
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError *resultError = nil;
        BOOL isSuccess = ([self createDirectoriesForFileAtPath:toPath error:&resultError] && [[NSFileManager defaultManager] copyItemAtPath:[self absolutePath:path] toPath:[self absolutePath:toPath] error:&resultError]);
        if (isSuccess) {
            if (success) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    success();
...
+(void)moveItemAtPath:(NSString *)path toPath:(NSString *)toPath success:(void(^)())success faild:(void(^)(NSError *error))faild
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError *resultError = nil;
        BOOL isSuccess = ([self createDirectoriesForFileAtPath:toPath error:&resultError] && [[NSFileManager defaultManager] moveItemAtPath:[self absolutePath:path] toPath:[self absolutePath:toPath] error:&resultError]);
        if (isSuccess) {
            if (success) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    success();
...
+(NSString *)pathForApplicationSupportDirectory
{
    ...
    //  王通 如果没有，就创建
    [YWFileManager createPathIfHaveNot:path];
...
{
    return [[YWFileManager pathForApplicationSupportDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[YWFileManager pathForCachesDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[YWFileManager pathForDocumentsDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[YWFileManager pathForLibraryDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[YWFileManager pathForMainBundleDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[YWFileManager pathForTemporaryDirectory] stringByAppendingPathComponent:path];
}
...
+ (void)createPathIfHaveNot:(NSString *)path
{
    //  王通 如果没有，就创建
...
+ (NSString *) fixApplicationFilePath:(NSString *)path
{
    ...
    NSRegularExpression *regex = [[NSRegularExpression alloc] initWithPattern:@"(?<=/Applications?/)\\w{8}(?:-\\w{4}){3}-\\w{12}(?=/)" options:NSRegularExpressionCaseInsensitive error:nil];
    
    NSString *temppath = NSTemporaryDirectory();
    NSTextCheckingResult *match = [regex firstMatchInString:temppath options:0 range:NSMakeRange(0,[temppath length])];
    NSString *appid = [temppath substringWithRange:[match range]];
    
    NSString *newPath = [regex stringByReplacingMatchesInString:path options:0 range:NSMakeRange(0,[path length]) withTemplate:appid];
    return newPath;
}
